...
### Round 2 — Phase 6 only — 2026-08-10 — same reviewer, resumed — **READY (converged)**

Zero blocking findings. All three round-1 blockers verified resolved against the
file, with the geometry re-run numerically: a hardness-ordered table with split
pairs (offsets 0 / 2.5 / 4.0 / 5.8 / 6.2 / 7.2) makes all 15 direction pairs'
head triangles disjoint inside the footprint with the u-turn cap respected — the
gate is satisfiable within the stated constraints, with slack. The cap
re-derives (15 − 2·3.78 = 7.44), and the new `import.rs` citation in the
footprint clause is verbatim-real.

Two new findings, both non-blocking, both accepted and folded in:

1. **The ≤3-subset gate already decides OQ-7.** Pairs are subsets and offsets
   are set-invariant, so a gate-passing implementation has all 15 pairs
   disjoint, which entails every size disjoint — OQ-7 closes itself the day the
   suite goes green. Folded into OQ-7 as an expectation rather than a
   resolution; it still wants a human to read the green run.
2. **"Every mirror pair split" is stronger than the geometry demands** —
   `left`/`right` are across-disjoint at any shared fork; only the slights need
   splitting. Kept uniform for tidiness, with the freedom recorded in §2.12.2
   for the in-app tuning pass.

**Converged at zero blocking in two rounds. Phase 6 `reviewed: 2026-08-10`;
the phase may now be planned.**

### Round 1 — Phase 6 only — 2026-08-10 — fresh reviewer (clean-context subagent) — **NOT READY (3 blocking)**

**Round 0, recorded per §7.0** (new episode — one appended phase): the phase
produces the observable directly — the turn arrow is paint a reader of the
figure sees, and three directions currently render as a star that states
nothing about the junction. Paint on the approach is the settled answer to how
a junction shows its turns (lane arrows Phase 4), so this fixes that picture
rather than adding a panel or a file. Proceed.

**Scope:** Phase 6 plus the sections drafted with it (§2.12 and subsections,
OQ-7, the CORRECTED note on Phase 3's As-built bullet). Shipped phases not
re-litigated.

The reviewer's grounding pass found the citations, integration order,
sibling-spec premises and gate literals (41, 63, 68) all sound. What failed was
the geometry itself — all three blockers trace to one underspecified point,
what the offset does with an equal-hardness mirror pair:

1. **The headline gate was unsatisfiable under the phase's own rule.** "Harder
   turns fork earlier" gives the two slights (±30°, equal hardness) the same
   fork, and two ±30° heads sharing a fork intersect at every fork position on
   the shipped proportions — measured numerically, not argued. Fixed by
   replacing the formula with a **per-direction offset table** sibling to
   `BRANCH_BEARING`, every mirror pair split, values decided in the app within
   three stated constraints (`through` 0, all ≥ 0, u-turn ≤ TAL − 2·reach).
2. **"Single-branch arrows are byte-identical" contradicted set-invariance** —
   jointly unsatisfiable with the mutation clause and trio disjointness. Fixed:
   only a lone `through` is byte-identical, a lone `left` moves too, and the
   gate says so.
3. **§2.12.4's six-heads packing proof was false.** Re-derived: 2.04 vs 2.64
   band-widths — they fit by the draft's own measure, and the shared-fork
   premise is what the fix abolishes. §2.12.4 rewritten to claim no
   impossibility beyond three and record the failed arithmetic; OQ-7 widened to
   four-through-six.

**Six non-blocking findings, all accepted, none rejected:** the "contained but
not disjoint" ambiguity at sizes 4–6 (now "contained only, no disjointness
assertion in either direction"); the hook's radius is `(reach − headHalf)/2`
and fork-independent, both sites corrected; the quoted `TURN_ARROW_LENGTH`
doc-comment phrase was already removed by Phase 5 — and since the footprint
clause proves the stagger stays inside ±TAL/2, the comment moved to "Not
touched" with the reason; the footprint test
(`holds the shaft still, and inside the footprint, whatever the directions`)
named as a must-pass-unedited gate clause, it being what caps the u-turn's
offset; `u_turn` has no bearing to key on — resolved structurally by the table;
and §2.7's "every branch leaving the shaft's far end" gains its dated CORRECTED
pointer **at ship time**, the sentence still describing the paint until then
(a timing distinction the reviewer accepted: Phase 3's bullet lost its
*decision* at drafting time, §2.7 loses its *description* only when the code
changes).
