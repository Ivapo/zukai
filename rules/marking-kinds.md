...
it for the whole link and is the odd one out in almost every section below.
`Hatching` alone is out of scope and paints a placeholder bar, as do the two kinds
whose fresh payload is empty.

### What the renderer asks for

`markingForm(doc, marking, offsets)` is the **one call the renderer makes**, and
it answers the only question with two answers: a marking is drawn either `across`
the road at a point or `along` it for the whole link. The kind branch lives there
rather than in `Diagram.tsx`, so the marking layer keeps its shape — one call, one
skip, one element.

`markingAnchor` is the `across` arm and returns `{ at, dir, span }`: a point on
the drawn polyline, the unit direction of travel, and the strip painted across.
`span` is one `laneBands` entry, or `{ offset: 0, width }` summed **from the
bands** rather than taken as `roadWidth - ROAD_MARGIN`, which would run the casing
lip through a needless float round trip. Every point kind's builder takes the
anchor and nothing else. `laneLine` is the `along` arm and takes no anchor at all.

**The anchor has exactly one kind-aware line**: a lane-less `turn_arrow` takes the
**nearside** band rather than the lane region, because an arrow has no
carriageway-wide meaning. It lives here rather than in `markingArrow` so the hit
target and halo move to lane 0 with it. It does **not** flip `dir` — an arrow
measured back from the junction still points the way the road runs.
...
all asphalt, below the junction glyphs, because a pad is the intersection's own
surface and paint under one is genuinely covered. Nesting inside `RoadShape`'s
`<g>` is wrong **twice over**: that group carries `onLinkPointerDown`, so a nested
marking's clicks would route to link selection; and a road drawn after its
neighbour would paint over the neighbour's markings. The class token comes from
the model (`kind.type.replace(/_/g, "-")`).
...
| `lane_line` | `.marking-line` + a style token | the whole link — not drawn from the anchor |
| `text` | `.marking-text` | the only kind that is not a path, and the only one at an angle |
...
the tiling, not a clamp**: `spanCells` derives the cell *count* from the span and
lets the pitch follow, so cells tile it exactly and no shape can reach the verge.
`MARKING_PITCH` is `LANE_PX / 3` — **one** rhythm for both tiled kinds, a third
rather than a half because two teeth to a lane read as two arrows.
...
painting nothing leaves an object findable only by accident.

**The hit target and halo are the anchor's transverse bar for every kind that sits
at a point**, so selection feels identical and a `stop_line`'s markup is
byte-for-byte what Phase 1 emitted. **The bar takes no `vector-effect`**, unlike
`.jn-stopbar` and the roads' hairlines: those are a symbol and hairlines and want
to hold their weight as the canvas zooms, while a marking is paint on a road and
scales with it — which leaves the layer byte-identical between canvas and export.
**`.marking-halo` is butt-capped** where `.road-halo` is round: same rule (a halo
matches the shape it highlights), but round caps here balloon past the lane the
marking spans.
...
- **It is the one kind drawn as two elements**: stems stroked, heads filled. A
  single filled outline would close the hook across its own chord and fill the
  half-disc inside it; a single stroked path would leave the heads hollow.
- **The stem's `stroke-width` is an attribute, not a rule** — a fraction of the
  band, so an arrow in a narrow ramp lane is a narrower arrow.
- **`ARROW_REACH` is the whole containment rule**, which is also why the **hook's
  radius is derived rather than picked**: the spec proposed a quarter of the band
  width, but that puts the return leg at the band edge before the head is added.
  `2R + headHalf = reach` instead, so one number bounds all six directions.
...
`back` paints branches at the **upstream** end pointing upstream. The case that
earns it is the **two-way left-turn lane**. Nothing imports one — `network.yaml`
has no per-lane direction at all — so it is schematic-only, as `Lane.kind` is. It
is a **field on the existing variant**, and a **`Vec` with `skip_serializing_if`**
rather than an `Option<Vec>`: empty and absent are then the same document.
...
- **So there is exactly one flip, and it is a frame rather than a sign change.**
  ...
  rotation about the **band centre at the marking's position**, carrying `stub`,
  `hook` and `head` over wholesale. The builders take an `ArrowFrame` rather than
  closing over one. The rotation centre is the band centre, **not** `anchor.at`,
  which sits on the polyline — they differ on every lane whose offset is not zero,
  and a test written about the wrong one fails a *correct* implementation.
- **The shaft shortens to `-fork → fork` iff a rear branch was actually built**,
  not iff the array was non-empty, which keeps `TURN_ARROW_LENGTH` the footprint
  either way.
...
`0`); `lane >= n-1` names nothing and **draws nothing**; otherwise
...
`Diagram` computes it once, and `boundaryTaken` is the comparison.
`laneLineOffsets` takes **no** `offsets` argument — which boundary a line sits on
is a fact about the cross-section, not about where the road was dragged. **The
centreline replaces too**: on a 2-lane road the lane region's centre *is* boundary
`0|1`, so the two Span entries must behave alike — which is also why
`boundaryTaken` has a tolerance, a centreline arriving as a literal `0` against an
...
- **A double line is the one marking that is not white.** Yellow says opposing
  ...
  the strokes reads as one fat line with a scratch down it. Decided in the app.
...
to the decoration list. For a 3-lane arterial the glyph's bar sits at `rp + 4` and
the junction's **hit disc** at `rp + 2`, which is also why a click much closer
drags the glyph instead of placing paint.
...
non-finite `position`, or a `lane` past the link's lanes. Each returns `undefined`
and the renderer emits *nothing* — indexing `laneBands` out of range would yield
`NaN` coordinates, which SVG renders as an invisible-but-corrupt path no `d=`
assertion catches. A skipped lane line **takes no divider with it** either,
because `laneLineOffsets` runs the same `boundaryOffset` and finds the same
nothing.
...
permanently**. Signs Phase 1 paid that cost by embedding Overpass Mono
(`rules/diagram-export.md`). The constraint has not been repealed but *satisfied*,
and it still governs every future glyph.
...
- **It is the one thing in the drawing set at an angle**, and it earns it: paint
  is on the road, so it turns with the road. There is deliberately **no upright
  flip** — a westbound road paints text upside down on screen and the right way up
  for the driver it is aimed at.
- **Centred across the band by arithmetic, not `dominant-baseline`**, whose
  support in a rasterized SVG is precisely what fails silently in the PNG path.
...
- **Empty content draws the placeholder bar**, which is why it can join the picker
  at all — and it is exactly what `needsText` counts, so font and glyph cannot
  disagree.
...
`Marking`s either, for the same reason, and belong on `GoreShape` — OQ-4.)

`strokeAllowance` (`export.tsx`) needed **no** change and `export.test.ts`
confirms it for every kind: every marking is painted inside the road it belongs
to, the allowance is already half the widest road, and the bar's 4-unit stroke is
what the `2` floor was sized for. The teeth, the zebra and the text are *fill*,
which `getBBox` measures with no allowance at all.
